...
\footnote{Find both on \vpageref{citadel}.}
...
\Person{\NPC{\F\Hu}{\Pgls{greyLibrarian}}{long sleeves smeared across an eternally running nose}{sings `\textit{Nobody knows the trouble I've seen\ldots}'}{to find that missing cat}}%
  {{0}{0}{-1}}% BODY
  {{2}{-2}{0}}% MIND
  {
...
\begin{enumerate}
  ...
  The book tells the tale of the \gls{town} \gls{warden} family.
...
\histEvent{260}{3}{As people began to migrate from \gls{archwarp}, \glsentrytext{town}'s \glsentrytext{warden} started to transport away much of the stone to build a citadel and improve the walls of \glsentrytext{town}}
...
\begin{speechtext}
  A tasteful tavern, and an industrial inn, with only the best of clientele.
  Of course, this means you're not coming in!
\end{speechtext}
...
\boxPair{
  ...
  \begin{dlist}
    ...
    \item
    \Gls{greyLibrarian} sits in a corner, reading a book about \gls{lostcity}.
    \item
    \Gls{townmaster} plays ridiculous games outside.
    \item
    \Gls{alchemist} plays a game of go with \pgls{whiteplains} cattle trader.
    \item
    \Gls{keras} plays a game of go with another gnome, from \gls{oolery}.
    \item
...
\Person{\NPC{\Gn\E}{Tompf}{cleans glasses (refuses to explain what `glasses' are)}{wanders off quietly, while people are talking}{to never feel the sting of boredom}}%
  {{1}{3}{2}}% BODY
  {{2}{1}{1}}% MIND
  {%
    \set{Academics}{1}
