...
\newcommand\hardestRiddleEver{
  \begin{speechtext}
    There are three gods, who will not speak your language.

    One speaks only the truth, one speaks only lies, and one selects a random answer.

    You know their language's words for `yes', and `no', but do not know which is which.

    You may ask three questions to identify each god.
  \end{speechtext}
}
