\thread[town,Roads,Forest]{Entitlement}
\label{entitlement}

\histEvent{06}{2}{\Glsfmttext{whiteplains} \glsfmtplural{warden} banish \glsfmttext{banditking}.
He and his brother -- \glsfmttext{sewerking} -- move to \glsfmttext{valley}}
...
\segment{Roads}% AREA
{Rotten Breath}% NAME
{\Glsentrytext{traitor} sends the party to find him a cure for his disease, contracted when robbing \glsfmttext{bskulls} from \glsfmttext{oldTemple}}% SUMMARY
\label{rottenBreath}
...
{\Glsentrytext{sewerthief} asks the troupe how they really feel about their position in wider society, to see if they might want to join the \glsfmttext{whiteBandits} crew}% SUMMARY
...
\Gls{sewerthief} wants a promotion, so he has taken on the dangerous mission of tracking down \pgls{griffin} eyrie.
To survive the night, he lights two fires in two places, then crawls far away, 
\ifcase\thetemperature%
  into a mound of snow.
\or%
  into a daffodil patch.
\or%
  among thistles and long grass.
\or%
  under a weeping willow.
\or%
  to nestle into the tall, dry, grasses.
\else%
  into wilting rhododendron bushes.
\fi%
...
\begin{speechtext}
  ...
  `From \gls{sixshadow} to \gls{archwarp}', they said it went, though nobody remembers where those are any more.
...
\segment{Roads}% AREA
{New Guards}% NAME
{\Glsfmtplural{whiteBandits} have an argument with the \glsentrytext{guard}, tensions rise, and the troupe must pick a side or de-escalate}% SUMMARY
...
\segment{town}% AREA
{Illegal Songs}% NAME
{The \glsentrytext{sunGuard} catch a bard singing about the wonders of \glsentrytext{lostcity}, and arrest him}% SUMMARY
...
If the \glspl{pc} manage to clear \gls{warningbard} of criminal charges, he probably won't lead them to the secret \glsfmtname{whiteBandits}.
And seeing \gls{townmaster} react so suddenly and irrationally won't tell the \glspl{pc} that \gls{townmaster} knows about \gls{shadowGate}, and how his ancestor had it decommissioned in order to charge steep river-taxes.

\begin{speechtext}
  %\textcolor{background}{\eighthnote~\raisebox{6pt}{\quarternote}}
  In a town where the priests shone so bright,
  \textcolor{background}{\eighthnote~\raisebox{3pt}{\twonotes}}

  \noindent
  Golden jewellery adorned their delight!
  \textcolor{background}{\quarternote\raisebox{-4pt}{\eighthnote~}\eighthnote}

  \textcolor{background}{\twonotes~\raisebox{-12pt}{\twonotes}}

  \noindent
  They all wanted a green-glowing mushroom,
  \textcolor{background}{\eighthnote~\raisebox{2pt}{\quarternote}}

  \noindent
  Though my meaning, let's not assume!
  \textcolor{background}{\eighthnote~\raisebox{-7pt}{\quarternote}}

  %\textcolor{background}{\quarternote\raisebox{5pt}{\eighthnote~}\eighthnote}
\end{speechtext}
...
See \autopageref{talismanIndex} for the full timeline.}
...
\gls{warningbard} will end up in the cells under the \nameref{guardstation} for three \glspl{cycle} (\pageref{guardstation}).
Of course, there's not a lot they \emph{can} do, unless they want to do something dangerous.

\segment[\gls{morning}]{Forest}% AREA
{The Beast's Treasure}% NAME
{\Glsfmttext{banditking} hunts for \glsfmtplural{ingredient}, and talks about justice}% SUMMARY
...
In the distance, the \glspl{pc} hear \gls{banditking} shouting encouragements at his archers as they hunt \localMonster\ (use the same stats as before, \vpageref{whiteBanditEntourage}).

\banditking\label{banditking}
\boxPair[b]{
...
\begin{speechtext}
  Adventurers!
  Or maybe just dogs, owned by the \gls{templeOfBeasts}?

  You know, \gls{lostcity} lies out here somewhere?
  If you reclaim humanity's treasures, will you keep it, or give it obediently to some \gls{jotter}?

  You're out here, risking your lives to keep \gls{valley} safe.
...
{Dead Nobles}% NAME
{\Glsentrytext{banditking} kills an entire \glsentrytext{warden} family, and investigation reveals \glsentrytext{alchemy}}% SUMMARY

We start with \glspl{warden}' bodies floating down a river, so if the troupe will not pass any river which lies downstream from \pgls{village}, skip this scene, and return to it later.
...
Whether they come from \pgls{village} or (more likely) an inner hamlet, the people who served them never liked them, and will not feel ready to co\"operate with the \glspl{pc}.

\paragraph{Searching for bandit tracks}
may lead the \glspl{pc} to the bandits' lair, (\vpageref{banditLair}) though
following them will take some time.%
...
{\Glsentrytext{sewerthief} gauges how the troupe feel about dead \glsfmtplural{warden}}% SUMMARY
...
\vpagerefnearby{southSmith}{}{\footnote{Find his \gls{statblock} \vpageref{sewerthief}.}}
...
He plans a meeting with them in a random tavern, chats a bit, then requests they relocate to \gls{pig}, where he introduces them to \gls{sewerking}.
