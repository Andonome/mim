...
The \gls{shadowGate}%
\footnote{Three centuries ago, the \gls{deep} gnomes in the \gls{shadowVault} created the \gls{shadowGate} --- \pgls{artefact} gateway to \gls{archwarp} which would only open when someone answered one of its riddle.
...
\footnote{\Gls{warden} Klept Grey lost a lot of money to the \gls{shadowGate}, as people used it to bypass his river (and the tax on goods).
He made a deal with the gnomes in the \gls{shadowVault}; they would disable the gateway, and he would help them build a warren next to \gls{town}, where they could live safely.

The gnomes agreed, and a young \gls{keras} approached the \gls{shadowGate} and answered its riddle, but instead of entering, he complained that the riddle was ``too easy, insulting even''.
Then he switched the game, and asked the \gls{shadowGate} a fiendish riddle.
