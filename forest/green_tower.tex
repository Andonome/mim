...
\histEvent{295}{2}{Priests at the temple of \glsfmttext{eldren} discover a deep pit, where highly valuable \glspl{glowshroom} grow \glssymbol{eldren}}
...
\begin{description}
  ...
  \begin{itemize}
    ...
    \lootMagic $x 3$
    \item
    An ancient book: \emph{An Account of the Fingers}.
    \label{fingers:tower}\index{Books!Fingers}
    \item
    \lootMagic
    \item
    \lootTalisman
...
\begin{exampletext}
  ...
  Whenever the larger hobgoblins attacked, the \glspl{goldenPriests} would begin with \pgls{spell} to turn them semi-undead, and then kill them, creating \pgls{ghast}.
  Finally, they would use spells from the Mind Sphere to have the \gls{ghast} pick more mushrooms and place them in the chest.
...
The Pious \Gls{ghast} guards this tomb from inside a closet.
...
\mapPic{t}{Dyson_Logos/green_secret}{
...
\Person{\npc{\M\D}{The Pious \Glsfmttext{ghast}}}%
  {{3}{2}{1}}% BODY
  {{0}{1}{0}}% MIND
  {%
    \set{Melee}{1}
...
the Pious \Glsfmttext{ghast} animates and comes to kill them.%
...
The \gls{ghast} attempts to take the troupe by surprise
\exRef{core}{Core Rules}{sneakattack}
but the clanking armour makes him easy to notice (\roll{Wits}{Vigilance}, \glsentrytext{tn}~6).
...
\begin{boxtext}
  ...
  \footnote{The priests of \gls{eldren} who gained the highest honours of the \gls{temple} remained here to guard it forever.}
...
If the troupe manage to break the spell, the \glspl{goldenPriests} stop picking mushrooms, take some time to re-examine their unlives, and discuss (silently, with the unknowable communications of the undead) what they want to do, if anything.
...
\begin{boxtext}
  ...
  \footnote{The natural tunnel was further excavated in order to horde the priests' treasures.
The \glspl{glowshroom} filled up the chests, but the priests just kept piling them on top, and ordering their undead hobgoblins to do the same.}
...
The room also contains 11 undead goblins, standing ready to kill any goblins who come up from the tunnel, or just anyone who enters.
...
\ghast[\npc{\D\N}{Hobgoblin \Glsfmtplural{ghast}}]

\medbreak

\subsection{Wrapping Up}

If the \glspl{pc} haven't attempted any research, the area will leave the players confused, which could give them motivation to try some research.

\subsubsection{Faking Membership in the \Glsfmttext{templeOfSickness}}
will allow the troupe to explore the lower tomb, and maybe even steal.
However, the \gls{templeOfSickness} has changed a lot in the last few centuries, so mistakes are easily made.

Rolling \roll{Intelligence}{Academics} at \tn[14] will allow them entry (so they'll almost certainly need to gain a Bonus from using the \gls{paperGuild} in \gls{town}, \vpageref{paperGuild}).
The disguise will involve many things, including removing \glspl{armour} and \glspl{weapon}.
Success lets them take enough \glspl{glowshroom} for 10~Fire \glspl{ingredient} immediately, and one per week thereafter (each Margin grants another week of safe theft).
Failure means the priests attack.

\subsubsection{If \pglsfmtshort{goldenPriests} \Glsfmttext{goldenPriests} Dies}
...
Soon after, the goblins take \gls{redfall} (details \vpageref{redfallFallen}).
