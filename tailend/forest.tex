\thread[Forest]{Forest Interruptions}
\label{interruptions}

\segment{Forest}% AREA
{The Curious Crawler}% NAME
{A hungry \glsentrytext{crawler} digs at a gnome's stump}% SUMMARY
\index{Gnomes!Rescue}
...
{Impenetrable Bog}% NAME
{Just find another way round and don't eat anything}% SUMMARY

\begin{boxtext}
  The trees up ahead look strange, the air smells stagnant, and then the ground starts squelching.
  You've arrived at a swamp, blocking the way.
\end{boxtext}

The swamp stretches two miles North, and two miles South.
It's only one mile across, but crossing a swamp requires a boat, or a sturdy constitution.

\paragraph{Swimming through the swamp}
...
Remember to mark to swamp's position on the main map, \vpageref{Irina/greylands}.
...
\humantrader[\npc{\M\Hu}{Sunkborn Twolisk}]
\index{Twolisk}

\humantrader[\npc{\T[2]\M\Hu}{Dogwilt and Offcuts Newarp}]
\index{Newarp}
