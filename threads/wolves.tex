\thread[town,Roads,Forest]{Wolf Heads}
\label{wolfHeads}

\noindent
The \glspl{wolfhead} want to find \gls{shadowGate}, and use it to make money.
Unfortunately, they lack manpower, finances, and don't know where to look.
\Gls{southCook} has plans for gaining wealth (covered in \nameref{troubleAle}, \autopageref{troubleAle}).

Over the course of this \gls{thread}, the troupe will meet the various members of the \gls{wolfhead}, and form allies or enemies.

\segment[\squash]{town}% AREA
{Rumours of Glory}% NAME
{The troupe hear about the \glsfmtplural{wolfhead}}% SUMMARY

Traders, \glspl{weaver}, even some \glspl{proctor}, have been gossipping about the \glspl{wolfhead} saving \pgls{village} from
\ifcase\thetemperature%
  collapse when \glspl{digger} tunnelled underneath the fields, walls, and houses.
  The \gls{wolfhead} dug them out, laid poisons, and cast \glspl{spell} which could reach into the deepest, smallest, holes.
\or%
  a dozen \glspl{crawler}, attacking Twolisk.
  The \glspl{wolfhead} used \glspl{spell} to freeze the webs, making them shatter.
  And once the icy pieces surrounded the \gls{village}, they began to melt, and the \glspl{crawler} became stuck in their own webs.
  Their leader had already organized for archers to focus the arrows on a single target until it died.
\else%
  \pgls{basilisk} that climbed over the \gls{village}'s walls, and begun tearing apart rooves.
  One of them had \pgls{spell} to gather every hearth and candle-flame inside the walls into a great, red, whip to sting the \gls{monster}, while the others jumped from rooftops to strike it in the eyes.
\fi%

Before the \gls{village} could celebrate, the \gls{wolfhead} began \gls{harvesting} their bounty, and sold the lot for a generously low price.
The \glspl{pc} probably don't care that this violates the \gls{templeOfBeasts}'s right to a 50\% cut on all sales of \gls{monster} produce, but everyone will chide them about it anyway.

\begin{speechtext}
  \rotatebox{-2}{\Glspl{guard} always take too long to arrive when}
  \rotatebox{-2}{something comes out the forest.}
  \\
  \rotatebox{2}{\underline{I say} whoever draws a beast's}
  \par\noindent
  \rotatebox{2}{blood first gets rights to the spoils.}

  \slshape\large
  I heard he summoned a flame in the shape of a dragon.
  Or maybe he summoned a dragon, made out of flame?
  \par\small\normalfont
  \bfseries Not being cheeky, just that yer a bunch of drunken shithead, y'know?
  Fair's fair, eh?
  You think fair's fair?
  \par\scriptsize
  Well isn't it\ldots?
  \par\mdseries
  How come you lot are making three \glsentrylongpl{gp} a day, and we still have \pgls{crawler} wandering over my mum's roof every night?
  \rotatebox{2}{I hear the \gls{witch} works for the \gls{templeOfCuriosity}.}
  \rotatebox{2}{He can come round here, and cure my curiosity, eh?}
\end{speechtext}

While \gls{town} develops the tale, start the next available \gls{segment}.
...
A poster refers to \gls{southRogue}.
Combine this \gls{thread} with the next, or run the next immediately.

\paragraph{If the \glspl{pc} hunt for \gls{southRogue},}
they will find her eventually (just keep running through the \glspl{thread}).
...
\segment{Roads}% AREA
{New Friends}% NAME
{\Glsentrytext{southRogue} follows the troupe to find out what the local \glsfmttext{guard} know about \glsfmttext{lostcity}}% SUMMARY
...
the \gls{jotter} will begin writing a letter to \gls{southDale}'s \gls{templeOfBeasts}, and ask the troupe to keep a \textit{close} eye on her in the meantime (he doesn't want her hanging round his \gls{broch}).
...
\begin{boxtext}
  ...
  Silhouettes of people's heads sit round it.
\end{boxtext}

\Gls{southSmith} has hired eight people from the nearest \gls{village} to join him (at 150~\glspl{cp} per day), to hunt for \gls{lostcity} past the \gls{edge}.
Night has fallen, and despite the dangers, the farmers have decided to light a fire, talk and cook.
None of them know how close they've come to the road; of course, the farmers are aware of the dangers, and are keeping \pgls{vigil}.
...
\begin{exampletext}
  ...
  \footnote{They now understand there were multiple towns, not just a single, large city.}
\end{exampletext}
...
\southSeeker[\setcounter{spentMP}{4}]
\showStdSpells[
  \stepcounter{diceNo}
]
...
The \gls{wolfhead} can then \gls{ambush} the \glspl{pc} once they emerge from \gls{shadowVault}.
