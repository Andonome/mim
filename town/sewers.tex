\section[\glsfmtplural{diggers} Catacombs]{\Glsfmtname{diggers}}
\label{sewers}

\histEvent{100}{3}{\Glsfmttext{warden} Fortima Grey (\glsentrytext{townmaster}'s great-grandmother) commissions underground tunnels for transportation and storage.
The local \glsfmttext{templeOfSickness} use some sections as a catacomb}
...
\histEvent{2}{11}{%
  \Glsfmtplural{sunGuard} attack the \glsfmttext{diggers} catacombs, and set fire to bedding.
...
\paragraph{The stench of death}
can induce vomiting.
The \glspl{pc} should roll \roll{Strength}{Medicine}, or empty their last meal onto the ground, gaining \pgls{ep}.%
\footnote{For extra verisimilitude, you might ask what the \gls{pc} last ate.}
This is \pgls{bandAct}, and each failure increases the \gls{tn} by 1 for the other \glspl{pc}.
On a tie, they manage to be discrete, and do not raise the \gls{tn} for others.

\mapentry[farmExit]{\Glsentrytext{traitor}'s Basement}

This tunnel connects the \nameref{sewers} to \gls{traitor}'s house, just outside \gls{town}'s walls.%
\footnote{See \nameref{smugglerHouse} \vpageref{smugglerHouse}.}
...
Beating \tn[8] means they find a map of \nameref{old_alchemy_basement} (\vpageref{old_alchemy_basement}), while hitting \tn[11] means they find a map of \gls{oldTemple} (\vpageref{necromancers_lair}).
\index{Maps!Elvish}
...
\widePic{Sebastiaen_Vrancx/tunnels}

\boxPair[t]{
  \label{newlyMintedGhastsI}
  \newGhast[\npc{\T[3]\D}{Ghasts}]
}{
  \newGhast[\npc{\T[3]\D}{Ghasts}]
}
...
\begin{exampletext}
  ...
  The \gls{bskulls} has no clue about preservation \glspl{spell}, so the new undead rot just like any other corpse, which is why the catacombs stink of death.

  The \glspl{diggers} plan to have the \glspl{ghast} storm \gls{town}'s citadel, once they have enough
  (see \vpageref{ghastPlan} for the plan).
\end{exampletext}
...
\begin{nametable}{Cell Contents}
  ...
  5th & 2 ghasts jump out, and one flees towards \nameref{farmExit}, \vpageref{farmExit}. \\
  6th+ & 5 ghasts jumps out. \\
\end{nametable}
...
Consult the chart (no matter which door the \glspl{pc} open first, that's the first door), and pick some \glspl{ghast} from the \glspl{statblock} \vpagerefrange{newlyMintedGhastsI}{newlyMintedGhastsII}.
...
they find the dead are silent when not active, and don't respond to noise, as they are deaf.%
...
Eventually, the \glspl{diggers} will breach this wall, and enter the larder below \gls{citadel}'s kitchen (left wing, \gls{area}~\vref{citadel:kitchen}).
...
The frigid water inflicts \pgls{ep}, and the filth demands a \roll{Strength}{Medicine} roll (\tn[10]) to avoid contracting
\iftoggle{judgement}{Corpse Hands (see the book \textit{Judgement}, \autopageref{Corpse Hands})}{a disease which slowly paralyses them}.
...
\paragraph{However, if \gls{sewerking} has rescued his brother}
(see \autopageref{banditsCaught}), he finds out about this tunnel, then later traps it by placing two ghasts down here, chained to heavy weights at the bottom of the \nameref{sewerWaterHall}.
The ghasts were recently \glspl{warden} of Twolisk.
\index{Twolisk}

\boxPair[t]{
  \label{newlyMintedGhastsII}
  \newGhast[\npc{\T[3]\D}{Ghasts}]
}{
  \newGhast[\npc{\T[3]\D}{Ghasts}]
}
...
\begin{boxtext}
  The door opens to \pgls{keeper}'s room, in striking contrast to the tunnels around.
...
The room contains a map of the local \glspl{region} (check the handouts), a Wolf-Run potion, encrypted letters%
\exRef{core}{Core Rules}{cryptography}
from \gls{traitor} (they discuss smuggling, \tn[13] to decrypt), encrypted letters to \gls{traitor} (unsent, suggesting plans to debilitate the local \gls{templeOfBeasts} by assassinating most \glspl{jotter}),%
\footnote{The assassination begins in \nameref{risingTitles}, \vpageref{risingTitles}.}
and many books.

The books cover history (real and imagined), \textit{The Art of Lies} (by an elvish author --- `Erend\"e'), and instructions on hosting a dinner party (it takes 280 pages to say `use seasoning`, and `have an anecdote').
...
\begin{boxtext}
  ...
  The walls hold wreaths of fresh flowers.
...
At night, this place contains 30 people, and the air is funky.
During the day, it contains 20.

Most of these people have no idea about the grader plan to fill the catacombs with ghasts.
They only know that they should never remove the bars from the doors, because those tunnels stink of death.
...
\index{Grey Family!Starvale}
Of course, \gls{townmaster} has no idea that Starvale comes here, and the
undertown residents have no idea who his father is.  If a player makes note of
the strange name, tell them that the boy sounds a bit strange (he's putting on
a fake lower-class accent).
...
the \glspl{diggers} from room \ref{underGuard} arrive (assuming they weren't killed already) with kindling and \glspl{torch}, and begin lighting fires.
...
Failure means that \pgls{interval} later, they receive \pgls{ep}, then 2~\glspl{ep} on the next, and so on, until 6~\glspl{ep}.
They can cure the poison with \pgls{elixir} made from an Air \gls{ingredient} (\tn[10] to prepare).
