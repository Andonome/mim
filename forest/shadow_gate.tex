...
\begin{boxtext}
  ...
  Soon they learned the local language (which they referred to as `Up') and began work around \gls{shadowVault}, cleaning, fixing, documenting the \glsfmtplural{artefact}, and crafting a impervious gate to ensure nothing else could emerge from the Vore Gate.}
\end{boxtext}
...
\footnote{The word `clear', can mean a lot of different things, especially when judged by a literally-minded mind, constructed by \gls{alchemy}.}
...
\begin{boxtext}
  Within the murky water, at the bottom, an orb glows.
\end{boxtext}

If the \glspl{pc} feel about,
...
\footnote{Having two alchemical gateways in the building means trouble, so the \gls{templeOfPoison} commissioned yet another \gls{artefact}.
...
Grabbing the orb hurts it, and it loses \pgls{mp} at every contact except the gentlest.
...
Pulling it out requires a \roll{Dexterity}{Larceny} roll (\tn[11]).
...
\footnote{The gnomes created this one-way door in secret, out of sheer resentment.
They didn't want to go back, but also hated that the \gls{wheatGuild} held the only key to access the Vore Gate.}
...
\begin{boxtext}
  Swimming through the murky water, you soon find air above --- stale air, barely breathable.
  And to one side, a staircase, leading up to a dry platform.
  Stumbling over a shallow rock-lip, you feel sand under your feet.
...
\Gls{shadowGate}%
\footnote{Three centuries ago, the \gls{deep} gnomes in \gls{shadowVault} created \gls{shadowGate} --- \pgls{artefact} gateway to \gls{archwarp} which would only open when someone answered one of its riddle.
The \gls{wheatGuild} charged traders a steep price to use the gateway, and the gnomes gained long-term employment answering riddles.}
lies underwater, but it can communicate as it always does --- by casting a \textit{Warp Detailed Earth} \gls{spell} to shift the sands on the raised platforms at the side of the room and write letters.%
\footnote{\Gls{warden} Klept Grey lost a lot of money to \gls{shadowGate}, as people used it to bypass his river (and the tax on goods).
He made a deal with the gnomes in \gls{shadowVault}; they would disable the gateway, and he would help them build a warren next to \gls{town}, where they could live safely.

The gnomes agreed, and a young \gls{keras} approached \gls{shadowGate} and answered its riddle, but instead of entering, he complained that the riddle was ``too easy, insulting even''.
Then he switched the game, and asked \gls{shadowGate} a fiendish riddle.
It could not answer, and since then, it would ask everyone that riddle --- and only that riddle --- but none could answer.}

%Over \gls{cTwo}, the frozen water will hide the \gls{shadowGate} entirely.
%The \glspl{pc} can roll \roll{Strength}{Crafts} (\tn[10]) to smash through this ice, which of course releases the ooze.

If anyone answers the riddle,%
\footnote{The real challenge for the \glspl{pc} is to find the gnome who created this riddle. Players cannot solve it, unless you assume they can't solve it, in which case, they will.}
it lets them pass with gratitude, and from then on begins to cycle through different (more reasonable riddles).
...
\begin{exampletext}
  ...
  But with the guardians sleeping peacefully (\vpageref[above]{shadowGuardian}), the proprietor didn't want to leave them unprotected (even with the gate present).
