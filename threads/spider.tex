...
\begin{boxtext}
  A trader caravan pulls up.
  Most seem worried, one seems infectiously happy as she talks quickly about escaping a near-death encounter.%
  \footnote{\Gls{spiderqueen} sent her \glspl{crawler} after the trader, along with illusionary \glspl{crawler}.
  The real ones became a little confused, so the plan didn't amount to much.}

  \begin{speechtext}
  I just stopped by a stream --- near the \glsentrytext{edge} --- to let the horses drink --- my name's `Barkridge' --- and I heard a funny song.
  Next thing I know --- two-dozen \glspl{crawler} came out.
  I sell \glspl{weapon}.
  Anyway I ran and they didn't catch up.
  I got food too!
  \end{speechtext}
\end{boxtext}

\paragraph{If the troupe ask Barkridge about the song,}
...
\begin{boxtext}
  A shepherd and four \glspl{guard} approach along the road.
  He shouts out ``hey!'', as his sheep rush off the road and into the forest.
  A distant song entices the sheep to run toward its discordant melody.
  The music sounds like it's decaying, like an overly ripe apple.
\end{boxtext}
...
\begin{boxtext}
  The sheep go beyond sight, and into the distant trees, then the song stops, and they begin to cry out in a way you've never heard sheep cry before.
  Half of them flee straight back out of the forest.
\end{boxtext}

\paragraph{If the characters pursue,}
they encounters webs%
\footnote{\Gls{spiderqueen} has gathered more \glspl{crawler}, and she needs to feed them again, so they have laid out their webs, and await their dinner.}
(\roll{Wits}{Vigilance} (\tn[9]) to spot), then see the \glspl{crawler} feasting on the sheep.
...
the \glspl{pc} spot all webs immediately.
...
\spell{Spider Hive}% Name
  ...
  {Each \gls{crawler} has Body \glspl{attribute} at +\arabic{spellPlusOne}, and vanishes once anything touches them.}
...
\Glspl{crawler} don't usually reach the inner hamlets -- they get distracted long before by the \glspl{broch}' twilight pipes or the sounds of the \glspl{village}.
...
\boxPair[b]{
  \humansoldier[\NPC{\glsentrysymbol{sylf}\M\Hu}{Bilefen Liskill}{long, dark hair hides missing ear}{purses lips}{to get promoted to builder}]
  \index{Liskill}
}{
  \begin{nametable}{What they see\ldots}
...
\begin{exampletext}
  ...
  However, Leta is the decision-maker, and her nose is longer than \gls{keras}'s (which gnomes consider to be an important point) so she says there's no use talking to the troupe without testing if they really can fight \glspl{crawler}.
...
The final plan is to cast an illusion of \pgls{crawler} and see how the troupe react.
If they appear as skilled warriors, the gnomes approach and tell them the plan to defeat \gls{spiderqueen}.
Otherwise, the encounter ends, the gnomes pursue other plans.

The proposal begins with Leta negotiating like a trader.
...
If the \glspl{pc} ask how the gnomes know exactly where she is, they explain they have triangulated her position through her periodic singing.
...
accosts the troupe on the streets, asking how many \glspl{griffin} they've seen, how many others have seen, how many nests they've seen, and so on.
...
\begin{speechtext}
  ...
  Could you track down some \gls{crawler} eggs, and just spread them about the forest, until the bioarachnid balance restores?
\end{speechtext}
