...
\begin{enumerate}
  ...
  \hint{footprints throughout the forest show they went offroad just before the \gls{bothy}, so nobody would see their approach}.
  \item
  ...
  \footnote{\Gls{wrecan} normally takes the souls of those who die through hatred.  People sometimes try to make amends after death by leaving gifts with the dead, so they can show \gls{wrecan} their gifts.  Where they go after that, nobody really thinks about.}
  \item
...
\humanthief[\npc{\T[2]\E\Hu}{\arabic{noAppearing} \Glspl{digger}}]
...
\sqpart{Town}% AREA
{Bandits Caught}% NAME
{The \glspl{sunGuard} have captured \gls{banditking}}% SUMMARY
...
The \glspl{digger} recently discovered an underground tunnel which leads from the \nameref{stationDungeon}'s waste-shaft (\vpageref{dunGrate}), to the \nameref{sewers} (area \vref{sewerWaterHall}).

\Gls{sewerking} and some of the \glspl{digger} will wrap dry wood in cheese-cloth, swim up to to the grate, and light a fire.
Once the \nameref{stationDungeon} fills with smoke, the \glspl{sunGuard} will have to evacuate, leaving the \glspl{digger} a moment to jump up and start opening doors.
...
Otherwise, \gls{forestpriest} will crate \pgls{talisman} for them which creates a \textit{Bubble} spell, to save a lock-smith \gls{digger} from the smoke.
...
At this point, the primary arc of the story has so many forks that I've never planned this far ahead -- the players have likely yanked the situation so far this way or that, that you have to just stop, take stock of the factions' plans and resources, and start to build a resolution (while forgetting, for a moment, that the players have other plans).
