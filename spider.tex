\sidequest[Forest,Town,Villages]{\Glsentrytext{spiderqueen}'s Song}
...
\textbf{Background:}
\Gls{spiderqueen} wants her chitincrawlers to feed well (on the \glspl{pc}, since they're here), but doesn't want them to get hurt.
She begins by casting an illusion of seven chitincrawlers, so the \glspl{pc} will spot the fakery, and stop trying to resist so the real ones can eat them more easily.
To complete this illusion, she casts yet another illusion of a gnome sitting in a tree, so the \glspl{pc} will feel convinced that nothing they see is worrying.
...
The \glspl{pc} roll Wits + Vigilance to understand their environment.
...
\paragraph{If the \glspl{pc} get close to \gls{spiderqueen},}
...
\textbf{Background:}
Most \glspl{village} in the inner circle, closer to a town, feel safe.
...
\paragraph{If the \glspl{pc} push for Gregory to join them,}
...
However, Holly is the chief illusionist, and her nose is longer than \gls{keras}'s,\footnote{Gnomes consider this to be a very important point.} so she says there's no use talking to the troupe without testing if they really can fight chitincrawlers.
...
The final plan is to cast an illusion of a chitincrawler and see how the troupe react.
...
She attacks neighbouring \glspl{village} twice, then gains her fifth level of Polymorph, and decides to become an air spirit for a while.
