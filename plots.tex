...
\histEvent{0}{\thecycle}{Present day}
...
Have a glance `\nameref{forestChapter}' \vpageref{forestChapter}, with the \gls{thread} list and locations, then fold the page-corner over, so you can find the next \gls{segment} and location descriptions easily.
...
Fold the corner of \vpageref{roadsChapter} so you can check available \glspl{segment} quickly.
...
\begin{enumerate}
  ...
  \item
  Start in \nameref{cinderfilch} \vpageref{cinderfilch}, and check the first \gls{segment} available in the \gls{region} (\vpageref{roadsChapter}).
  \item
...
\begin{dlist}
  ...
  grew up in \gls{town}.
  \Gls{townmaster} sentenced them in \gls{court}, but won't recognize them.
    \item[Others]
  ...
  \begin{description}
    ...
    once lived in the \gls{deep}, and still don't speak \gls{tradeTongue} very well, though they lived in \gls{southDale} for a year before conscription in the \gls{guard}.
  \end{description}
  \item
  \begin{description}
...
\begin{enumerate}
  ...
  \item
  Check the next available \gls{thread} \gls{segment} (\vpageref{roadsChapter}).
\end{enumerate}
