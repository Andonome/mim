...
The second time, he won't ask -- he will sue them in the \gls{court} (find both \vpageref{citadel}).%
...
It exists to stop \glspl{woodspy} slithering into \gls{town}.
...
her brother's in jail (room  \vref{spyBroth} in the \nameref{stationDungeon}), and \gls{alchemist} hasn't returned his book on \textit{Delightful Riddles: A Dinner Party Anthology} (\gls{area} \vref{citadel_alchemist}).
\label{paperCat}
\index{Riddles!Books}

\medbreak

\subparagraph{Researching the local area}
might bring up a map of the whole of \gls{valley} (\roll{Intelligence}{Survival}, \tn[14]).

\subparagraph{Researching old temples}
of \gls{eldren} might bring up the map of \gls{necromancer}'s temple, from before it crumbled (\roll{Intelligence}{Empathy}, \tn[10]).

\subparagraph{Asking the young people who frequent the library about what they are doing}
reveals a young man who has mapped all of \gls{town} (\roll{Intelligence}{Empathy}, \tn[10]).

\subparagraph{Books on \gls{alchemy}}
never have `\gls{alchemy}' on the cover: instead, \glspl{spell} come as part of a larger work, making them difficult to find.
...
\subparagraph{Researching \gls{lostcity}}
yields a map of an old \gls{paperGuild} building in \gls{archwarp} (\roll{Wits}{Academics}, \tn[18]).

\subparagraph{Researching old elvish settlements}
yields a map.
...
Its multi-level structure, and the fact that piles of books form a kind of pyramid shape mean that anyone on the upper levels has an excellent view of everyone below them.
...
If the \glspl{pc} ever search \gls{citadel}, roll $1D6$ in each hallway as the \glspl{pc} move left or right.
If a roll repeats, skip it and use the next undiscovered roll above (when moving right) or below (when moving left).

\begin{exampletext}
  The \glspl{pc} pose as servants to hunt for a book they think \gls{alchemist} might be hiding.
  They enter left wing's second floor (where they shouldn't be), and the \gls{gm} rolls: \dicef{4}~\dicef{3}~\dicef{3}.
  They listen at the doors of rooms 5 and 3 before opening the doors.
  The last number is a repeat, so the \gls{gm} bumps it up to a 4.

  Since they took the time to listen at the doors, the \gls{gm} rolls another die, to see who comes down the hall.
  \dicef{6}
  They hear footsteps coming, as a visiting \gls{seneschal} enters her room.
\end{exampletext}

Every so often, people move through the halls.
Roll $1D6$ to determine which room.
If someone's in that room, they leave; otherwise, someone comes down the hall to enter the room.

%!
\null
\begin{description}
  \item[The outer grounds:]
  Double doors, inside the courtyard, lead downstairs to the \gls{court}.
  \begin{itemize}
    \item
    Stalls of all kinds sell snacks.
    \item
    The double doors lead inside \gls{citadel}'s ground floor (right wing, \gls{area}~\ref{citadel:entrance}).
  \end{itemize}
  \item[\Glsfmttext{court}:]
  where the public witness justice.
  \begin{itemize}
    \item
    The viewing area, where people buy snacks (see the town market handouts).
    \item
    \Gls{townmaster}'s platform, with stairs up to \gls{area}~\ref{citadel:guardroom}
    \label{citadel:watch}
    \item
    Temporary prisons, leading into the \gls{court}.
    \item
    A massive storage room, full of empty cages, all waiting for \glspl{monster}.
    \item
    Hall of records, where \glspl{scrivener} note legal precedent as \gls{townmaster} creates it.
  \end{itemize}
  \item[Ground Floor:]
  where outsiders and servants come and go.
    \begin{description}
      \item[Left Wing:]
      where servants rush down a long hall.
      \begin{dlist}
        \item
        A secret door (which looks like any other wooden panel) leads to a small room with drinks.
        \item
        Light closet, with lanterns, lantern oil, candles and tinder boxes.
        \item
        Smoking room, with a visiting \gls{seneschal} from \gls{southDale} smoking a pipe.
        \item
        Guardroom, with stairs down to \gls{area}~\vref{citadel:watch}.
        \label{citadel:guardroom}
        $2D6$~\glspl{sunGuard} at all times.
        \item
        Ballroom, with stuffed \gls{griffin} centre-piece, and balconies looking down upon it.
        \label{citadel:ballroom}
        \item
        Grand stairway, leading up to the Left Wing's first floor (\gls{area}~\ref{citadel:updownStairs}).
      \end{dlist}
      \item[Right Wing:]
      full of tapestries.
      \begin{dlist}
        ...
        Servants' Quarters.
        \item
        Clothes washing room (big tubs, lots of ash and fat).
        \item
        Toilet, with jester smoking a pipe (exits outside the citadel, wide enough for someone with \pgls{weight} of 20 or less to fit through).
        \item
        Kitchen (fireplace leads up through the floor above, \gls{area}~\vref{citadel:games}).
        In the larder below, strange knocking sounds can be heard at night (see \gls{area}~\vref{citadelTunnel}).
        \label{citadel:kitchen}
        \item
        Double-doors to courtyard and stairs up to the first floor.
        Two \glspl{sunGuard} stationed here at all times.
        \label{citadel:entrance}
      \end{dlist}
    \end{description}
  \item[First Floor:]
  where the important people talk.
    \begin{description}
      \item[Left Wing:]
      ...
      \begin{dlist}
        ...
        Light closet, with lanterns, lantern oil, candles and tinder boxes.
        \item
        Stairway down to the ground floor and up to the second.
        \label{citadel:updownStairs}
        \item
        \Gls{townmaster}'s office, with records of sales and taxes.
        \item
        Hallway with doors to balconies, each overlooking \gls{area}~\vref{citadel:ballroom}.
        \item
        Library (provides +1 to any research).
        \index{Library!Citadel}
        \item
        Guest Rooms.
      \end{dlist}
      \item[Right Wing:]
      ...
      \begin{dlist}
        ...
        \item
        Games room
        (fireplace leads up through the floor above, \gls{area}~\ref{citadel:townmaster})
        \label{citadel:games}
        \item
        Sewing room.
        \item
        Toilet
        (exits outside the citadel, wide enough for someone with \pgls{weight} of 25 or less to fit through).
        \item
        Secret stairway up to the floor above (\gls{area}~\ref{citadel:shield}), behind a door that looks like a wooden panel
        (spot with \roll{Wits}{Crafts} at \tn[9]).
        \label{citadel:secretUp}
      \end{dlist}
    \end{description}
  \item[Second Floor:]
  with important people and valuable items.
    \begin{description}
      ...
      \begin{dlist}
        ...
        \item
        \Gls{alchemist}, the \gls{seeker}'s Study, with one \lootTalisman, and one \lootTalisman\ (description \vpageref{talismanIndex}).
        \item
        \Gls{alchemist}'s observatory, with telescope and model orrery.
        Looking through the telescope provides clues about upcoming \glspl{segment} (check what might be visible on the roads, \vpageref{roadsChapter}.)
        \item
        \Gls{seneschal} Flay's room (half-deaf and too old to do anything but keep the key to the treasury's second door (\gls{area}~\vref{citadel:treasury}).
        \item
        Lady Arsenika Grey's lounge and perfumery.
        \label{citadel:perfumery}
        \index{Grey Family!Lady Arsenika}
        \item
        A door to the right wing hall.
      \end{dlist}
      \item[Right Wing:]
      where ornate weapons line the walls.
      \begin{dlist}
        \item
        Secret staircase behind a massive shield (too large for a human to lift) leading one floor down (\gls{area}~\ref{citadel:secretUp}).
        \label{citadel:shield}
        \item
        \Glsentrytext{townmaster}'s room, with a key to the treasury's first door by his bedside candle.
        \label{citadel:townmaster}
        \item
        Treasury, with brass-braced wooden doors, and a gate beyond.
        Inside the floor is covered in moss which squeaks when stepped on%
        \iftoggle{judgement}{(see \textit{Judgement}, \autopageref{screechMoss})}{}.
        \label{citadel:treasury}
        \index{Treasury, \Glsfmttext{citadel}}

        The central table has a statue of an arachnid-like crab (\gls{weight}~3), made of a red crystal, which serves as 3 Fire \glspl{ingredient}.
        This statue came from the Fingers some centuries ago (covered \vpageref{bookOfFingers}).

        Various locked chests each contain $1D6 \times 100$ \glspl{sp}.
        \item
        A door to the left wing.
        \item
        Dressing room, with fancy clothes (worth $1D6$~\glspl{gp}, but increase the \gls{tn} to interact with a fence by 2).
        \item
        Watchtower, with wide windows overlooking both \gls{town} and the outer fields.
        \Pgls{sunGuard} takes notes on people entering \gls{town}.
      \end{dlist}
    \end{description}
\end{description}
...
he will return it only after composing an encrypted letter to \gls{greyLibrarian},%
\footnote{\Glsfmttext{greyLibrarian} has less interest in encryption than most \glsfmtplural{seeker}.
Specifically, she has zero interest.
She's never told \glsfmttext{alchemist} because she has no idea who keeps sending her `malformed pamphlets'.}
so the troupe can deliver the letter to her.
This process takes \pgls{interval}, and he requests the \glspl{pc} wait by saying `just five more minutes', every time they try to leave.

The book details a fiendish riddle, called `the infamous Riddle of \gls{shadowGate}', but does not explain any context.
\index{Books!Riddles of the \Gls{shadowGate}}
\index{Riddles!Books}
...
\begin{exampletext}
  When \gls{healerLeader} saw one-too-many people in serious need of help just lying in \gls{town}'s streets, and waiting to be flung into the \gls{guard} or the \gls{court}, he decided to do something.
  After finding unsatisfying answers for some years, \gls{forestpriest} eventually asked him to become part of the solution.

  The \glspl{diggers} give him poisonous mushrooms which paralyse people,%
...
Barkwind stands at the front door, ensuring good people are well met, and scruffy lowlifes don't enter.
...
\boxPair[t]{
  ...
  \begin{dlist}
    ...
    \index{Books!\Glsfmttext{lostcity}}
    \item
    ...
    \item
    \Glsfmtname{sunCaptain} and \glsfmtname{investigator} argue about which of them must take responsibility for bandit raids close to \gls{town}.
  \end{dlist}
}
...
\begin{enumerate}
  \item
  The drinking hall hosts \gls{village} \glspl{warden} playing games, and half a dozen \glspl{sunGuard} (sometimes including \gls{sunCaptain}).
  \label{horseHall}
  \item
  \label{horseKitch}
  ...
  \index{Elves!War on \gls{lostcity}}
...
\begin{itemize}
  ...
  \item
  A secret trapdoor in the kitchen leads down to a basement with barrels of \gls{southCook}'s `ale'.
  (\roll{Wits}{Crafts} at \tn[10])
  \item
  A further trapdoor in the basement leads down to the \nameref{sewers}, \gls{area}~\vref{farmExit}.
  (\roll{Wits}{Crafts} at \tn[10])
\end{itemize}
...
\Gls{sunCaptain} has an obsession with guards constantly rotating around the premise.
As a result, they've hidden a stash of whiskey in the nettle-bushes at the back, and sometimes have `rounds', while they do the rounds.

\smolMapPic[\Large]{Dyson_Logos/guard_station}{
  \ref{stationStables}/42/15,
  \ref{stationStorage}/81/427,
  \ref{stationStorage}/54/11,
  \ref{stationCaptainRoom}/63/11,
  \ref{stationToilet}/895/427,
  \ref{stationWeapons}/85/59,
  \ref{stationSleep}/86/14,
  \ref{stationDressing}/30/11,
  \ref{stationLecture}/86/81,
  \ref{stationRecords}/37/42,
  \ref{stationInterrogation}/285/57,
  \ref{stationShrine}/44/65,
  \ref{stationStairs}/53/42,
}

The wooden buildings tacked into the outer wall have thin rooves which constantly bend and creak --- walking silently across them is impossible for anything with a total \gls{weight} of 20 or more.
...
\begin{enumerate}
  ...
  \item
  Weapons.
  \label{stationWeapons}
  \item
  \Glsfmtname{sunCaptain}'s Room.
  \label{stationCaptainRoom}
  \item
  Lecture Hall (though mostly used as a drinking hall).
  \label{stationLecture}
  \item
  Records Room, containing lists of fugitives, laws, tax records (a copy is kept in \gls{townmaster}'s treasury), and paintings of local \glspl{warden}.
  \label{stationRecords}
  \item
  Interrogation room.
  \label{stationInterrogation}
  \item
  Simulacra of \pgls{woodspy}, \gls{crawler}, \gls{griffin}, (small) \gls{basilisk}, and bandit, made from wood and leather.
  The guards use these to explain combat tactics.
  \label{stationShrine}
  \item
  Stairway down to the dungeons (\vpageref{stationDungeon}).
  \label{stationStairs}
\end{enumerate}
