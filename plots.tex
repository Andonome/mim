...
define \gls{valley}'s \glspl{region}'s space -- \gls{town} sits in the centre, the Roads surround it, and the eternal Forest surrounds everything.
Look at the map \vpageref{Irina/greylands}; once the troupe steps onto the road, you can start \pgls{sq} by picking the first available \gls{segment}, \vpageref{sqListRoads}.

Nestled safely in the centre of all roads is \gls{town}.
If the \glspl{pc} visit here, you can check the first available \gls{segment} \vpageref{townChapter} (\nameref{townChapter}), then find the details for whichever place in town the troupe want to go in the same chapter.

An outer circle of dense forest surrounds everything; when the \glspl{pc} step away from the known lands, check the next available \gls{segment} \vpageref{sqListForest}.
