...
\noindent
The \gls{whiteBandits} and \glspl{diggers} instigate their plans to destabilise \gls{valley}, and the \glspl{pc} must pick a side --- either pro-establishment, or anti-establishment.
However, siding with the \gls{whiteBandits} does not mean siding with the \gls{diggers}, because both groups have different members and ideologies.
Whatever they decide, \gls{valley} will change.
...
{The bandits in the sewer cut \glsentrytext{sunCaptain}'s throat}% SUMMARY
...
\begin{exampletext}
  \Gls{sunCaptain} announced the previous day that he would begin harsh raids against the people living under \gls{town}.
...
they find \gls{sunCaptain} next to his wife, blood spilling from his neck.
...
Remember that whoever's trying to patch up \gls{sunCaptain}'s bleeding neck won't be able to join the chase.
...
\boxPair[t]{
  \newGhast[\npc{\T[7]\D}{\arabic{noAppearing} \Glsfmtplural{ghast}}]
}{
  \newGhast[\npc{\T[7]\D}{\arabic{noAppearing} \Glsfmtplural{ghast}}]
}
...
\segment{town}% AREA
{The Dead Rise}% NAME
{\Glsfmttext{sewerking} releases \glspl{ghoul} upon \glsentrytext{citadel}, killing \glsentrytext{townmaster}}% SUMMARY
...
\begin{exampletext}
  ...
  They waited for the \gls{bskulls} to curse them with a touch of undead (so the \glspl{ghast} would not attack them), then laid out wretched kindling for \gls{citadel} -- dry twigs, green branches, rotten meats, and bone.
...
they find \gls{sunGuard} fleeing (and trying not to be seen) and about 100 \glspl{ghast} in \gls{citadel}, devouring the face of anyone they encounter.
...
{The \glsentrytext{sunGuard} request the troupe clear \glsentrytext{citadel} of \glsfmtplural{ghast}, as the old order return}% SUMMARY

If \gls{townmaster} dies, his eldest surviving son, Constance, takes over leadership (with the help of \gls{sunCaptain}).
...
\footnote{The \gls{sunGuard} explain that, while the \glspl{ghast} exist within \gls{citadel}, \gls{citadel} counts as `beyond the \gls{edge}', and is therefore not their jurisdiction.}

Twenty \glspl{ghast} remain inside \gls{citadel}, while another thirty remain in the catacombs.
Both groups of \glspl{ghast} will attack en mass, rather than splitting up.
