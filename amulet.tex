...
Therefore, he requested that \gls{alchemist} send the item to the \gls{paperGuild} in \gls{southDale}, while \gls{traitor} arranged to have the woodspy bandits steal it en route by extortion.
...
He pulls the \glspl{pc} into his office (see page \pageref{guardstation}), and tells them the score.
...
\paragraph{If the \glspl{pc} try to understand the item,}
have them roll Intelligence + Academics, TN 10.
Success means they understand the forgery.%
%! Understanding items ref required.

\paragraph{Once the \glspl{pc} reach the crossroads,}
they can see the simple \gls{guard} camp in the distance, and hear the woodspy bandits coming behind them.

\paragraph{If the \glspl{pc} aren't wearing \gls{guard} uniforms, do not carry visible weapons, and simply stroll along quietly,}
...
The book comes from the restricted works in the \gls{paperGuild} (i.e. the local library).
...
\paragraph{If the \glspl{pc} catch up,}
...
The thief will have to pay a library fine of 3 cp (unless the \glspl{pc} push for a harsher penalty).

\paragraph{If the \glspl{pc} try to understand the importance of the book,}
...
\begin{boxtext}
  \begin{quotation}
    ...
    But we also celebrate, as the famous lizardite amulet, property of \gls{paperGuild} of \gls{southDale}, was rested from the hands of the bandits.
