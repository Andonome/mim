...
The old \gls{healer} stretched deep underground, and soon after it was built, a library was commissioned by local alchemists.
...
The top skull has the family's name carved into the forehead, and speaking the family name along with religious incantations to \gls{eldren} activates some magical effect.

Each pillar can be used once a day.
They require a short prayer (which takes some time to complete), and understanding which prayer aligns with which skull requires an \roll{Intelligence}{Academics} Group Roll (the TN depends upon the type of pillar).
...
\begin{enumerate}
  ...
  \item{Target regenerates $1D6+2$ \glspl{fp}.}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item{The target regains $1D6+3$ \glspl{fp}.}
